...
export class EventDetailPage {
  ...
  share(event, type){
  	switch(type){
  		case 0: 
  			this.socialSharing.shareViaFacebook(this.event.name, "", 'https://communityopportunity.com/events/detail/index.php?id='+ this.event.id).then(() => {
			...
			break;
		case 1: 
  			this.socialSharing.shareViaTwitter(this.event.name, "../assets/share.png", 'https://communityopportunity.com/events/detail/index.php?id='+ this.event.id).then(() => {
			...
			// this.emailComposer.isAvailable().then((available: boolean) =>{
			 // if(available) {
			   let email = {
				  ...
				  body: 'https://communityopportunity.com/events/detail/index.php?id='+ this.event.id,
				  isHtml: true
				};
